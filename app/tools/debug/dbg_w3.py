...
from api.w3.erc20_contract import ERC20Contract
from api.w3.router_contract import TCRouterContract
from api.w3.token_list import TokenListCached, StaticTokenList
                ...
                for tx in txs:
                    if tx.is_of_type(ActionType.SWAP):
                        if tx.dex_aggregator_used:
                            print(tx.tx_hash, f' = {tx.full_volume_in_rune:.1f} Rune; ', tx.dex_info, tx, file=f)
                            print(tx.tx_hash, tx.full_volume_in_rune, 'Rune')
                            f.flush()
