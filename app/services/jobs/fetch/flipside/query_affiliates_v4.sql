...
base4 as (
  select
    ...
    case
      ...
      when affiliate_address = 'rg' then 'RangoExchange'
      when affiliate_address = 'ej' then 'EdgeWallet'
      when affiliate_address = 'ds' then 'DefiSpot'
      when affiliate_address in ('ti', 'td', 'te') then 'TrustWallet'
